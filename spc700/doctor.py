...
from spc700 import environment  # noqa: E402
...
from spc700 import models  # noqa: E402
from spc700.memory import SparseMemory  # noqa: E402
...
def _package() -> Finding:
    """The distribution, named so it cannot be mistaken for the part.

    The package and the part it models share a name, so labelling both with it
    puts two different things under one word in a report somebody else has to
    read.
    """
    return Finding("package", True, f"spc700 {VERSION}")


def _default_build(name: str) -> Cpu:
    """One part of that name, on a store that allocates nothing until it is read.
...
def report(found: Sequence[Finding]) -> list[str]:
    ...
    lines = [f"spc700 {VERSION} on {platform.python_version()}, {platform.system()}", ""]
