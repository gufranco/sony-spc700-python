...
def machine(code, base=0x0200, fill=0, **registers):
...
def run(code, steps=1, **registers):
...
class ResetTest(unittest.TestCase):
    def test_the_program_counter_comes_from_the_reset_vector(self):
    ...
    def test_the_registers_are_not_assumed_clear(self):
        cpu = core.Cpu(Memory(fill=0))

        self.assertNotEqual((cpu.a, cpu.x, cpu.y), (0, 0, 0))

    def test_a_reset_repeats_for_one_seed(self):
    ...
    def test_a_different_seed_gives_a_different_machine(self):
...
class StatusTest(unittest.TestCase):
    def test_every_flag_survives_a_round_trip(self):
        cpu, _ = machine([0x00])
    ...
    def test_no_flag_set_reads_back_as_zero(self):
        cpu, _ = machine([0x00])
    ...
    def test_the_page_flag_moves_the_direct_page(self):
        cpu, _ = machine([0x00])
    ...
    def test_the_direct_page_is_the_bottom_page_without_it(self):
        cpu, _ = machine([0x00])
...
class MoveTest(unittest.TestCase):
    ...
    def test_a_direct_to_direct_move_leaves_the_flags_alone(self):
    ...
    def test_transferring_the_stack_pointer_into_the_index_sets_flags(self):
        cpu, _ = run([0x9D], sp=0x00)

        self.assertTrue(cpu.z)

    def test_transferring_the_index_into_the_stack_pointer_leaves_flags(self):
...
class ArithmeticTest(unittest.TestCase):
    ...
    def test_addition_brings_the_carry_in(self):
    ...
    def test_addition_sets_the_half_carry_at_the_nibble(self):
    ...
    def test_a_comparison_leaves_the_accumulator_alone(self):
    ...
    def test_a_bitwise_and_keeps_the_common_bits(self):
        cpu, _ = run([0x28, 0x0F], a=0x3C)

        self.assertEqual(cpu.a, 0x0C)

    def test_a_bitwise_or_keeps_every_bit(self):
        cpu, _ = run([0x08, 0x0F], a=0x30)

        self.assertEqual(cpu.a, 0x3F)

    def test_an_exclusive_or_drops_the_common_bits(self):
        cpu, _ = run([0x48, 0x0F], a=0x3C)

        self.assertEqual(cpu.a, 0x33)

    def test_incrementing_wraps_at_the_top(self):
        cpu, _ = run([0xBC], a=0xFF)

        self.assertEqual(cpu.a, 0x00)
        self.assertTrue(cpu.z)

    def test_decrementing_wraps_at_the_bottom(self):
        cpu, _ = run([0x9C], a=0x00)

        self.assertEqual(cpu.a, 0xFF)
        self.assertTrue(cpu.n)

    def test_a_nibble_exchange_swaps_the_halves(self):
...
class WordTest(unittest.TestCase):
    def test_a_word_load_takes_two_bytes(self):
    ...
    def test_a_word_subtraction_borrows_between_the_bytes(self):
    ...
    def test_a_word_comparison_leaves_the_pair_alone(self):
    ...
    def test_a_word_increment_carries_into_the_high_byte(self):
    ...
    def test_a_word_decrement_borrows_from_the_high_byte(self):
...
class MultiplyDivideTest(unittest.TestCase):
    def test_a_multiply_fills_the_register_pair(self):
        cpu, _ = run([0xCF], y=0x10, a=0x10)

        self.assertEqual((cpu.y << 8) | cpu.a, 0x0100)

    def test_a_multiply_sets_flags_from_the_high_byte(self):
        cpu, _ = run([0xCF], y=0x00, a=0x00)

        self.assertTrue(cpu.z)

    def test_a_divide_gives_a_quotient_and_a_remainder(self):
        cpu, _ = run([0x9E], y=0x00, a=0x0A, x=0x03)

        self.assertEqual(cpu.a, 0x03)
        self.assertEqual(cpu.y, 0x01)

    def test_a_divide_by_zero_does_not_raise(self):
...
class DecimalTest(unittest.TestCase):
    def test_a_decimal_adjust_after_add_carries_at_nine(self):
        cpu, _ = run([0x60, 0x88, 0x01, 0xDF], steps=3, a=0x09)

        self.assertEqual(cpu.a, 0x10)

    def test_a_decimal_adjust_after_subtract_borrows_at_zero(self):
        cpu, _ = run([0x80, 0xA8, 0x01, 0xBE], steps=3, a=0x10)

        self.assertEqual(cpu.a, 0x09)

    def test_a_decimal_adjust_above_ninety_nine_adds_sixty_and_carries(self):
        cpu, _ = run([0xDF], a=0xAA, psw=0x00)

        self.assertEqual(cpu.a, 0x10)
        self.assertTrue(cpu.c)

    def test_a_decimal_adjust_with_the_carry_already_set_adds_sixty(self):
        cpu, _ = run([0xDF], a=0x11, psw=core.FLAG_C)

        self.assertEqual(cpu.a, 0x71)

    def test_a_decimal_adjust_reads_the_accumulator_it_just_changed(self):
        cpu, _ = run([0xDF], a=0x9A, psw=0x00)

        self.assertEqual(cpu.a, 0x00)
        self.assertTrue(cpu.c)

    def test_a_decimal_adjust_leaves_a_valid_pair_alone(self):
        cpu, _ = run([0xDF], a=0x42, psw=0x00)

        self.assertEqual(cpu.a, 0x42)

    def test_a_decimal_subtract_above_ninety_nine_takes_sixty_off(self):
        cpu, _ = run([0xBE], a=0xAA, psw=core.FLAG_C | core.FLAG_H)

        self.assertEqual(cpu.a, 0x44)
        self.assertFalse(cpu.c)

    def test_a_decimal_subtract_with_the_half_carry_set_leaves_the_nibble(self):
        cpu, _ = run([0xBE], a=0x42, psw=core.FLAG_C | core.FLAG_H)

        self.assertEqual(cpu.a, 0x42)

    def test_a_decimal_subtract_without_the_half_carry_takes_six_off(self):
...
class IndexComparisonTest(unittest.TestCase):
    def test_an_index_comparison_leaves_the_index_alone(self):
        cpu, _ = run([0xC8, 0x10], x=0x20)

        self.assertEqual(cpu.x, 0x20)
        self.assertTrue(cpu.c)

    def test_an_index_comparison_of_equals_sets_zero(self):
        cpu, _ = run([0xC8, 0x20], x=0x20)

        self.assertTrue(cpu.z)

    def test_the_other_index_compares_the_same_way(self):
        cpu, _ = run([0xAD, 0x20], y=0x20)

        self.assertTrue(cpu.z)
        self.assertEqual(cpu.y, 0x20)

    def test_an_index_comparison_below_clears_the_carry(self):
...
class AddressingGuardTest(unittest.TestCase):
    def test_a_mode_that_names_no_address_is_refused(self):
        cpu, _ = machine([0x00])
...
class BranchTest(unittest.TestCase):
    def test_an_unconditional_branch_moves_the_program_counter(self):
        cpu, _ = run([0x2F, 0x02])

        self.assertEqual(cpu.pc, 0x0204)

    def test_a_branch_taken_on_zero_moves(self):
        cpu, _ = run([0xE8, 0x00, 0xF0, 0x02], steps=2)

        self.assertEqual(cpu.pc, 0x0206)

    def test_a_branch_not_taken_falls_through(self):
        cpu, _ = run([0xE8, 0x01, 0xF0, 0x02], steps=2)

        self.assertEqual(cpu.pc, 0x0204)

    def test_a_branch_on_bit_set_is_taken(self):
    ...
    def test_a_branch_on_bit_clear_is_taken(self):
    ...
    def test_a_compare_and_branch_is_taken_when_they_differ(self):
    ...
    def test_a_decrement_and_branch_is_taken_until_zero(self):
    ...
    def test_a_decrement_and_branch_on_the_index_stops_at_zero(self):
...
class ControlTest(unittest.TestCase):
    def test_a_jump_moves_the_program_counter(self):
        cpu, _ = run([0x5F, 0x00, 0x03])

        self.assertEqual(cpu.pc, 0x0300)

    def test_an_indexed_indirect_jump_reads_its_target(self):
    ...
    def test_a_call_pushes_the_return_address(self):
    ...
    def test_a_return_takes_the_address_back_off(self):
        cpu, _ = run([0x3F, 0x05, 0x02, 0x00, 0x00, 0x6F], steps=2)

        self.assertEqual(cpu.pc, 0x0203)

    def test_a_page_call_jumps_into_the_top_page(self):
        cpu, _ = run([0x4F, 0x40])

        self.assertEqual(cpu.pc, 0xFF40)

    def test_a_table_call_reads_its_vector(self):
    ...
    def test_a_break_takes_its_vector_and_sets_the_break_flag(self):
    ...
    def test_a_return_from_interrupt_restores_the_status(self):
...
class StackTest(unittest.TestCase):
    ...
    def test_the_stack_pointer_wraps_inside_its_page(self):
        cpu, memory = run([0x2D], sp=0x00, a=0x11)

        self.assertEqual(cpu.sp, 0xFF)
        self.assertEqual(memory.read8(0x0100), 0x11)

    def test_the_status_register_survives_a_push_and_pull(self):
...
class BitTest(unittest.TestCase):
    def test_setting_a_bit_leaves_the_others_alone(self):
    ...
    def test_setting_the_top_bit_uses_the_opcode_index(self):
    ...
    def test_clearing_a_bit_leaves_the_others_alone(self):
    ...
    def test_a_test_and_set_leaves_the_bits_of_the_accumulator(self):
    ...
    def test_a_test_and_clear_removes_the_bits_of_the_accumulator(self):
    ...
    def test_a_carry_load_takes_the_addressed_bit(self):
    ...
    def test_a_carry_store_writes_the_addressed_bit(self):
    ...
    def test_an_inverted_carry_load_takes_the_complement(self):
    ...
    def test_a_bit_complement_flips_the_addressed_bit(self):
...
class FlagTest(unittest.TestCase):
    def test_the_carry_can_be_set_and_cleared(self):
        cpu, _ = run([0x80, 0x60], steps=2)

        self.assertFalse(cpu.c)

    def test_the_carry_can_be_complemented(self):
        cpu, _ = run([0xED], psw=0x00)

        self.assertTrue(cpu.c)

    def test_the_overflow_flag_clears_the_half_carry_with_it(self):
        cpu, _ = run([0xE0], psw=core.FLAG_V | core.FLAG_H)

        self.assertFalse(cpu.v)
        self.assertFalse(cpu.h)

    def test_the_page_flag_can_be_set_and_cleared(self):
        cpu, _ = run([0x40, 0x20], steps=2)

        self.assertEqual(cpu.direct_page, 0x0000)

    def test_the_interrupt_flag_can_be_set_and_cleared(self):
...
class HaltTest(unittest.TestCase):
    def test_a_sleep_stops_the_processor(self):
        cpu, _ = run([0xEF])

        self.assertTrue(cpu.stopped)

    def test_a_stop_stops_the_processor(self):
        cpu, _ = run([0xFF])

        self.assertTrue(cpu.stopped)

    def test_a_stopped_processor_stays_where_it_is(self):
    ...
    def test_a_run_that_never_settles_is_stopped(self):
...
class EveryOpcodeTest(unittest.TestCase):
    def test_every_opcode_executes(self):
