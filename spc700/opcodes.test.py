...
class TableTest(unittest.TestCase):
    ...
    def test_every_entry_names_an_instruction_a_mode_and_a_size(self):
        for mnemonic, mode, size in opcodes.OPCODES:
            self.assertTrue(mnemonic)
            self.assertTrue(mode)
            self.assertIn(size, (1, 2, 3))

    def test_the_disassembly_templates_cover_the_whole_byte(self):
        self.assertEqual(len(opcodes.TEXT), 256)

    def test_a_size_matches_its_template_operands(self):
    ...
    def test_bit_indexed_instructions_share_one_mnemonic(self):
    ...
    def test_the_table_call_targets_are_all_one_mnemonic(self):
        for opcode in range(0x01, 0x100, 0x10):
            self.assertEqual(opcodes.OPCODES[opcode][0], "tcall")


class BitIndexTest(unittest.TestCase):
    def test_a_bit_instruction_takes_its_index_from_the_opcode(self):
    ...
    def test_a_clear_takes_the_same_index_as_its_set(self):
        self.assertEqual(opcodes.bit_index(0x12), 0)
        self.assertEqual(opcodes.bit_index(0xF2), 7)

    def test_a_table_call_takes_its_index_from_the_opcode(self):
...
class DisassembleTest(unittest.TestCase):
    ...
    def test_a_listing_runs_past_a_return_when_not_asked(self):
        listing = opcodes.disassemble(bytes([0x00, 0x6F, 0x00]), 0, 0x0200)

        self.assertEqual(len(listing), 3)

    def test_the_address_wraps_at_the_top_of_the_space(self):
