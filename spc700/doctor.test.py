...
DECLARED: dict[str, Any] = {
    "suites": [{"name": "spc700", "path": "v1", "files": 4, "commit": "ebe1875d48f374"}]
}
...
def a_cache(files: int) -> tempfile.TemporaryDirectory[str]:
    ...
    where = Path(held.name) / "spc700" / "v1"
...
class ExaminationTest(unittest.TestCase):
    ...
    def test_and_the_package(self) -> None:
        ...
        self.assertIn("spc700", found[0].detail)
    ...
    def test_the_package_line_is_not_confused_with_the_part_of_the_same_name(self) -> None:
        ...
        self.assertEqual(names.count("spc700"), 1)
    ...
    def test_a_part_that_will_not_build_is_reported_rather_than_hidden(self) -> None:
        ...
        found = [one for one in doctor.examine(build=boom) if one.name == "spc700"]

        self.assertFalse(found[0].ok)
        self.assertIn("Complaint: the core exploded", found[0].detail)

    def test_a_part_that_builds_reports_where_it_came_up(self) -> None:
        """Not that it built. Where it built, because that is what differs run to run."""
...
class DeclaredTest(unittest.TestCase):
    def test_what_the_definition_names_is_reported(self) -> None:
        ...
        self.assertIn("1 suites: spc700", found.detail)
...
class SuiteTest(unittest.TestCase):
    ...
    def test_a_suite_with_no_stated_file_count_is_taken_as_it_is(self) -> None:
        with a_cache(2) as where:
            found = doctor._suite({"name": "spc700", "path": "v1"}, Path(where))
...
class PathTest(unittest.TestCase):
    ...
    def test_the_version_is_read_out_of_the_file_rather_than_imported(self) -> None:
        found = re.search(
            r'VERSION[^"\']*"([^"]+)"', (doctor.ROOT / "spc700" / "version.py").read_text()
        )
