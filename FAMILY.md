...
Sony published no data sheet for this processor. The top rung of the authority
ladder is empty here and the record says so rather than promoting the rung below
it, so the settling is done by the artifact: 256,000 single step cases, one per
opcode per seed, each comparing every register, every access in order, and the
cycle each access fell on. That is 1,182,940 cycles compared, and a case that
disagrees about when an access happened fails exactly as loudly as one that
disagrees about what was read.

It is a clocked part. It runs a program, it is driven by a budget of cycles and
it reports what it spent, so the whole of the section about a clock applies.
