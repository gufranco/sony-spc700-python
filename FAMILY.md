...
Everything in the table above is in place. The core is held to Nintendo's own
command tables for the cycles every instruction takes, and to the pinned suite
for every cycle of every one of the 256 opcodes.

**Cycle accuracy, stated precisely.** 256,000 cases and 1,182,940 cycles
compared, zero failures. Every cycle's address and kind are compared, not the
count, and the value wherever the case asserts one. Separately and with no suite
on the machine, 225 documented instructions are assembled, stepped, and checked
against the figure Nintendo printed for each, plus both figures of all 28
conditional branches.

**What that claim does not cover**, recorded in
[`conformance/divergences.json`](conformance/divergences.json) with what would
settle each: the shape of a cycle rests on the recording alone, because the
manual gives a count and no breakdown, and a count cannot distinguish two cores
that touch different addresses in a different order. There is no Sony datasheet
for this part; Nintendo's appendix is a licensee-facing instruction set summary,
which is the best source known to exist and is not a description of silicon.

**One place the manual is wrong.** It gives `MOVW dp, YA` four cycles. The part
takes five, because a store reads its destination first and this one is no
exception. Read again at 450 dots per inch to rule out a misread digit. The
document is also uneven here: it gives the reading form five cycles for two reads
and the writing form four for two writes plus the same addressing work.

**Two rows that are not cycle counts at all.** `SLEEP` and `STOP` halt the part,
and the manual says on the same page that neither mode can be used on this
console. The seven cycles the suite records are where the recording was cut.

**A defect only the bus comparison could find.** Every store on this part reads
its destination and discards the byte. The state comparison had passed 256,000
cases without it, because the discarded byte goes nowhere.

**Three behaviours are deliberately not modelled**, each recorded with what would
change it: interrupts, because this core has no pin for one and the suite never
raises one; the reset state of the registers, which no source states and which
this core scrambles rather than clears so a program depending on one fails
visibly; and the rest of the S-SMP, the timers, the four ports and the boot ROM,
which belong to the unit around this processor.
